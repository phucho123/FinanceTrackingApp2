    ...
    const getIncomeList = async () => {
        try {
            ...
            const response = await axios.get(
                `${apiBaseUrl}/transactions?userId=${user._id}&type=Income&sortMoney=desc`
            );
            if (response.status === 200) {
                ...
                setError(response.data.message || "Get list of incomes failed");
            }
        } catch (error) {
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.timeContainer}>
                ...
                <Text style={{ color: "#00A86B" }}>dfsdfs</Text>
            </View>

            {incomeList && (
                <TouchableWithoutFeedback onPress={() => navigation.navigate("BudgetReport")}>
                    <View style={{ flex: 1, justifyContent: "space-around" }}>
                        <View style={{ alignItems: "center" }}>
                            <Text style={{ color: "#fff", fontSize: 32, fontWeight: "bold" }}>You Earned 💸</Text>
                            <Text style={styles.moneyTotal}>${calculateMoneyTotal(incomeList)}</Text>
                        </View>

                        <View style={styles.cartContainer}>
                            <Text style={styles.mainText}>Your biggest Income is from</Text>

                            <View style={styles.categoryContainer}>
                                {renderIcon(incomeList[0].categoryName)}
                                <Text style={styles.categoryName}>{incomeList[0].categoryName}</Text>
                            </View>

                            <Text style={styles.biggestMoney}>${incomeList[0].money}</Text>
                        </View>
                    </View>
